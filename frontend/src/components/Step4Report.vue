...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM updates, adjust scroll position to keep button at the same viewport position
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extract analysis query (bilingual)
    const queryMatch = text.match(/(?:分析问题|Analysis query|Query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extract mission scenario (bilingual)
    const reqMatch = text.match(/(?:预测场景|Mission scenario|Scenario):\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Extract statistics (bilingual)
    const factMatch = text.match(/(?:相关预测事实|Related facts):\s*(\d+)/)
    const entityMatch = text.match(/(?:涉及实体|Entities):\s*(\d+)/)
    const relMatch = text.match(/(?:关系链|Relations):\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extract sub-questions (bilingual)
    const subQSection = text.match(/### (?:分析的子问题|Sub-questions|Analysis sub-questions)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extract key facts (bilingual)
    const factsSection = text.match(/### (?:【关键事实】|Key Facts|\[Key Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Extract core entities (bilingual)
    const entitySection = text.match(/### (?:【核心实体】|Core Entities|\[Core Entities\])\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
    ...
    // Extract relations (bilingual)
    const relSection = text.match(/### (?:【关系链】|Relations|\[Relations\])\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/(?:当前有效事实|Active facts|Current facts):\s*(\d+)/)
    const histMatch = text.match(/(?:历史\/过期事实|Historical facts):\s*(\d+)/)
    ...
    // Extract active facts (bilingual)
    const activeSection = text.match(/### (?:【当前有效事实】|Active Facts|\[Active Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Extract historical facts (bilingual)
    const histSection = text.match(/### (?:【历史\/过期事实】|Historical Facts|\[Historical Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Extract entities (bilingual)
    const entitySection = text.match(/### (?:【涉及实体】|Entities|\[Entities\])\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
// Helper: check if text is a "no response" placeholder (bilingual)
const isNoResponseText = (text) => {
  if (!text) return true
  const t = text.trim()
  return t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' ||
         t === '(No response from this channel)' || t === '(No response)' || t === '[No response]'
}
...
const parseInterview = (text) => {
  ...
  try {
    // Extract interview topic (bilingual)
    const topicMatch = text.match(/\*\*(?:采访主题|Interview Topic):\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()

    // Extract interview count (bilingual)
    const countMatch = text.match(/\*\*(?:采访人数|Interviewed):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extract selection reasoning (bilingual)
    const reasonMatch = text.match(/### (?:采访对象选择理由|Selection Reasoning|Interview Selection)\n([\s\S]*?)(?=\n---\n|\n### (?:采访实录|Interview Records))/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse each person's selection reasoning
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          // Save previous person's reasoning
          ...
          // Continuation of reasoning (exclude summary paragraphs)
          currentReason.push(line.trim())
        }
      }
      
      // Save last person's reasoning
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Extract each interview record (bilingual)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (nameRoleMatch) {
        ...
        // Set this person's selection reasoning
        interview.selectionReason = individualReasons[interview.name] || ''
      }
      
      // Extract bio (bilingual)
      const bioMatch = block.match(/_(?:简介|Bio|Profile):\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question starts with "1.", needs special handling
      ...
      // Extract answers - split by channel/platform (bilingual)
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*(?:关键引言|Key Quotes):|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()

        // Separate Channel 1 / Channel 2 answers (bilingual)
        const twitterMatch = answerText.match(/(?:【Twitter平台回答】|【Deliberation Channel】)\n?([\s\S]*?)(?=(?:【Reddit平台回答】|【Interview Channel】)|$)/)
        const redditMatch = answerText.match(/(?:【Reddit平台回答】|【Interview Channel】)\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic (backward compat)
        if (!twitterMatch && redditMatch) {
          if (interview.redditAnswer && !isNoResponseText(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !isNoResponseText(interview.twitterAnswer)) {
      ...
      // Extract key quotes (bilingual)
      const quotesMatch = block.match(/\*\*(?:关键引言|Key Quotes):\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // First try matching > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: match > "text" or > \u201Ctext\u201D (curly quotes)
        if (!quoteMatches) {
    ...
    // Extract interview summary (bilingual)
    const summaryMatch = text.match(/### (?:采访摘要与核心观点|Interview Summary)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Extract search query (bilingual)
    const queryMatch = text.match(/(?:搜索查询|Search query|Query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extract result count (bilingual)
    const countMatch = text.match(/(?:找到\s*(\d+)\s*条|Found\s+(\d+)\s+results|Results:\s*(\d+))/)
    if (countMatch) result.count = parseInt(countMatch[1] || countMatch[2] || countMatch[3])

    // Extract related facts (bilingual)
    const factsSection = text.match(/### (?:相关事实|Related Facts|Facts):\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extract edges (bilingual)
    const edgesSection = text.match(/### (?:相关边|Related Edges|Edges):\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // Extract nodes (bilingual)
    const nodesSection = text.match(/### (?:相关节点|Related Nodes|Nodes):\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Mission Scenario: '),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Analysis sub-queries'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No entities found'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relations found')
      ])
    ])
  }
}
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if text is a placeholder (uses shared bilingual helper)
    const isPlaceholderText = (text) => isNoResponseText(text)
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "问题X:" or "Question X:" (Chinese/English format, backend new format)
      // 2. "1. " or "\n1. " (number+dot, legacy format)
      let matches = []
      let match

      // Try "Question X:" / "问题X:" format first
      const cnPattern = /(?:^|[\r\n]+)(?:问题|Question)\s*(\d+)[：:]\s*/g
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        props.result.interviews[activeIndex.value]?.selectionReason && h('div', { class: 'selection-reason' }, [
          h('div', { class: 'reason-label' }, 'Selection Reasoning'),
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (shown only when real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), since section title is already shown in outer wrapper
  ...
  // Fix non-consecutive ordered list numbering: keep incrementing when single-item <ol> are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // Try extracting content within <final_answer> tags
  ...
  // Try finding content after Chinese "最终答案:" (final answer)
