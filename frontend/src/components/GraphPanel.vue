    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing. Refresh the graph shortly.</span>
...
const entityTypes = computed(() => {
  ...
  const typeMap = {}
  // Military entity type color mapping (with fallback palette)
  const militaryColors = {
    'MilitaryUnit': '#004E89',   // Navy blue
    'Location': '#1A936F',       // Green
    'Objective': '#C5283D',      // Red
    'Threat': '#FF6B35',         // Orange-red
    'Asset': '#3498db',          // Light blue
    'Route': '#f39c12',          // Amber
    'SupplyPoint': '#27ae60',    // Emerald
    'TerrainFeature': '#7B2D8E', // Purple
    'CivilianEntity': '#E9724C', // Coral
    'Organization': '#9b59b6',   // Violet
  }
  const fallbackColors = ['#FF6B35', '#004E89', '#7B2D8E', '#1A936F', '#C5283D', '#E9724C', '#3498db', '#9b59b6', '#27ae60', '#f39c12']
  
  props.graphData.nodes.forEach(node => {
    const type = node.labels?.find(l => l !== 'Entity') || 'Entity'
    if (!typeMap[type]) {
      typeMap[type] = { name: type, count: 0, color: militaryColors[type] || fallbackColors[Object.keys(typeMap).length % fallbackColors.length] }
    }
    typeMap[type].count++
  })
...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward center, clustering isolated nodes in the center area
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset proportional to distance, ensuring curves are clearly visible
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier curve formula B(t) = (1-t)^2*P0 + 2(1-t)t*P1 + t^2*P2, t=0.5
