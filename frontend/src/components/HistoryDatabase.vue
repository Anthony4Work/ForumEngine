  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (uses first 20 chars of simulation requirement) -->
    ...
    <!-- Simulation replay detail modal -->
    <Teleport to="body">
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count for current row, ensuring each row is centered
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record the latest target state regardless of animation)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before executing to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
.grid-pattern {
  ...
  /* Position from top-left; height changes expand at bottom only, preserving existing grid positions */
...
/* ===== Simulation replay detail modal styles ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
