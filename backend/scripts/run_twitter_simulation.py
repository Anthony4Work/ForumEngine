...
Features:
- Does not close environment immediately after simulation, enters command-wait mode
...
class TwitterSimulationRunner:
    ...
    # Twitter available actions (excludes INTERVIEW, which can only be triggered manually via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    async def run(self, max_rounds: int = None):
        ...
        Args:
            max_rounds: Maximum simulation rounds (optional, truncates excessively long simulations)
        """
        print("=" * 60)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, truncates excessively long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    # Initialize log configuration (fixed filenames, clean old logs)
