...
class OasisProfileGenerator:
    ...
    def _search_graphiti_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Graphiti graph search to get rich information related to the entity

        Search edges and nodes separately then merge results.
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Graphiti edge search attempt {attempt + 1} failed: {str(e)[:80]}, retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Graphiti edge search failed after {max_retries} attempts: {e}")
            return None
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Graphiti node search attempt {attempt + 1} failed: {str(e)[:80]}, retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Graphiti node search failed after {max_retries} attempts: {e}")
            return None
        ...
        try:
            ...
            logger.info(f"Graphiti retrieval complete: {entity_name}, got {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 4. Use Graphiti retrieval for richer information
...
- 机构账号发言要符合其身份定位"""
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles as CSV format (conforming to OASIS official requirements)
