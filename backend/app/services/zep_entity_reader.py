...
Reads nodes from the Graphiti knowledge graph and filters entities matching predefined entity types
"""
...
class EntityReader:
    ...
    Main features:
    1. Read all nodes from the Graphiti knowledge graph
    2. Filter nodes matching predefined entity types (nodes with labels beyond just Entity)
    3. Retrieve related edges and associated nodes for each entity
    """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"{operation_name} still failing after {max_retries} attempts: {str(e)}")
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all edges related to a specific node (with retry mechanism)
        ...
        """
        try:
            # Call Graphiti API with retry mechanism
            edges = self._call_with_retry(
                func=lambda: run_async(GraphitiEntityEdge.get_by_node_uuid(self.graphiti.driver, node_uuid)),
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's Labels only contain "Entity", it does not match any predefined type; skip it
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity with full context (edges and related nodes, with retry mechanism)
        ...
        try:
            ...
            node = self._call_with_retry(
                func=lambda: run_async(GraphitiEntityNode.get_by_uuid(self.graphiti.driver, entity_uuid)),
