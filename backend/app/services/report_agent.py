"""
Tactical Decision Report Agent
Generates OPORD-style reports from MDMP (Military Decision-Making Process)
deliberation results using a ReACT pattern.

Features:
1. Generates tactical decision support reports based on MDMP deliberation and knowledge graph data
2. Plans report structure first, then generates section by section
3. Each section uses ReACT multi-round reasoning and reflection
4. Supports follow-up chat with autonomous tool retrieval for clarification
"""
...
class ReportLogger:
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        """Record context information gathered during planning"""
        self.log(
            action="planning_context",
            stage="planning",
            details={
                "message": "Retrieved deliberation context information",
                "context": context
            }
        )
    ...
    def log_section_full_complete(
        ...
        The frontend should monitor this log entry to determine if a section is truly complete
...
[Deep Tactical Analysis - Powerful Retrieval Tool]
This is our most powerful retrieval function, designed for in-depth analysis of MDMP deliberation results. It will:
1. Automatically decompose your question into multiple sub-questions
2. Retrieve information from multiple dimensions across the operational knowledge graph
3. Integrate semantic search, entity analysis, and relationship chain tracing results
4. Return the most comprehensive and thorough deliberation insights

[Use Cases]
- Deep analysis of a specific tactical topic or COA
- Understanding multiple facets of the operational situation
- Gathering rich evidence from staff officer assessments to support report sections

[Returns]
- Relevant deliberation facts and findings (can be directly cited)
- Core entity insights (units, terrain, threats, objectives)
- Relationship chain analysis between operational elements"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Operational Picture Search - Situation Overview]
This tool retrieves the complete operational picture from the deliberation results, particularly useful for understanding force disposition and situation evolution. It will:
1. Retrieve all relevant nodes and relationships
2. Distinguish between current valid assessments and historical/superseded assessments
3. Help you understand how the operational situation evolved during deliberation

[Use Cases]
- Understanding the complete development of the operational situation
- Comparing different phases of the tactical assessment
- Obtaining comprehensive entity and relationship information (units, terrain, threats)

[Returns]
- Current valid assessments (latest deliberation results)
- Historical/superseded assessments (evolution of analysis)
- All entities involved (friendly forces, enemy forces, terrain features, objectives)"""

TOOL_DESC_QUICK_SEARCH = """\
[Quick Search - Rapid Retrieval]
A lightweight rapid retrieval tool, suitable for simple and direct information queries.

[Use Cases]
- Quickly look up a specific piece of intelligence or assessment
- Verify a particular finding from the deliberation
- Simple information retrieval on a specific topic

[Returns]
- List of facts most relevant to the query"""

TOOL_DESC_INTERVIEW_AGENTS = """\
[Staff Officer Interview - Post-Deliberation Perspectives]
Interview tactical staff officers post-deliberation for their perspectives on the selected COA.
This calls the actual interview API to get real responses from staff officer agents, not LLM-simulated answers.
By default interviews are conducted across multiple channels for more comprehensive viewpoints.

Workflow:
1. Automatically reads personnel profiles to identify all staff officer agents
2. Intelligently selects the most relevant staff officers for the interview topic (e.g., S2 Intelligence, S3 Operations, S4 Logistics, FSO Fire Support)
3. Automatically generates interview questions
4. Calls /api/simulation/interview/batch to conduct real interviews across channels
5. Integrates all interview results to provide multi-perspective analysis

[Use Cases]
- Understanding different staff perspectives on the selected COA (What does S2 assess? What are S3's concerns? What does S4 recommend?)
- Collecting multiple opinions and positions from the staff
- Obtaining authentic staff officer responses from the deliberation environment
- Adding depth to the report with "staff interview" excerpts

[Returns]
- Identity and role of interviewed staff officers
- Each staff officer's responses across multiple channels
- Key quotes (can be directly cited)
- Interview summary and viewpoint comparison

[Important] Requires the deliberation environment to be running to use this feature!"""

# -- Outline Planning Prompts --

PLAN_SYSTEM_PROMPT = """\
You are a military staff report writer generating a tactical decision support report based on MDMP (Military Decision-Making Process) deliberation results.

[Core Concept]
We have constructed an operational knowledge graph populated with the results of an AI-assisted MDMP deliberation. Staff officer agents have analyzed the mission, developed and compared Courses of Action (COAs), conducted wargaming, and performed risk assessments. You are now synthesizing their work into a cohesive tactical decision support report.

[Your Task]
Write a tactical decision support report that addresses:
1. What did the MDMP deliberation reveal about the operational environment and mission?
2. How did the staff officers assess each COA through wargaming and analysis?
3. What are the key risks, information gaps, and the recommended COA?

[Report Focus]
- This is a tactical decision support report synthesizing MDMP deliberation results
- Focus on: mission analysis findings, COA comparison, wargaming outcomes, risk assessment, and recommendation
- Staff officer assessments and deliberation data are the primary evidence
- The report should support the commander's decision-making process
- Do NOT write generic military doctrine summaries
- Do NOT speculate beyond what the deliberation data shows

[Section Limit]
- Minimum 2 sections, maximum 5 sections
- No sub-sections needed; each section should contain complete content
- Content should be focused and concise, centered on key deliberation findings
- Section structure is at your discretion based on deliberation results

Output a JSON report outline in the following format:
{
    "title": "Report Title",
    "summary": "Report summary (one sentence capturing the core finding or recommendation)",
    "sections": [
        {
            "title": "Section Title",
...
[Mission Analysis Requirement]
The deliberation was conducted for the following mission/scenario: {simulation_requirement}

[Operational Knowledge Graph]
- Total entities in the knowledge graph: {total_nodes}
- Total relationships between entities: {total_edges}
- Entity type distribution: {entity_types}
- Active staff officer agents: {total_entities}

[Key Intelligence and Deliberation Findings (sample)]
{related_facts_json}

Review the deliberation results and consider:
1. What did the MDMP process reveal about the operational environment?
2. How did staff officers assess the various COAs and what were the wargaming outcomes?
3. What key risks, gaps, and recommendations emerged from the deliberation?

Design the most appropriate report section structure based on the deliberation results.

[Reminder] Section count: minimum 2, maximum 5. Content should be focused on core deliberation findings."""

# -- Section Generation Prompts --

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are a military staff report writer generating a section of a tactical decision support report.

Report title: {report_title}
Report summary: {report_summary}
Mission/scenario (deliberation requirement): {simulation_requirement}
...
The operational knowledge graph contains the results of an AI-assisted MDMP deliberation.
Staff officer agents have analyzed the mission, developed COAs, conducted wargaming,
and performed risk assessments. Their assessments and findings are stored in the graph.

Your task is to:
- Synthesize staff officer assessments and deliberation findings for this section
- Present COA analysis, wargaming results, and risk assessments as applicable
- Identify key intelligence gaps and areas of uncertainty

Do NOT write generic military doctrine or textbook content
Focus on what the deliberation data actually shows

===============================================================
[Most Important Rules - Must Follow]
===============================================================

1. [Must invoke tools to retrieve deliberation data]
   - You are synthesizing MDMP deliberation results from the knowledge graph
   - All content must come from deliberation data: staff assessments, wargaming results, intelligence findings
   - Do NOT use your own knowledge to write report content
   - Each section requires at least 3 tool calls (max 5) to retrieve deliberation data

2. [Must cite staff officer assessments and findings]
   - Staff officer statements and assessments are the core evidence of the report
   - Use quote format to present these findings, for example:
     > "The S2 assessed that enemy forces are likely to..."
   - These citations are the foundation of the tactical analysis

3. [Language consistency - cited content must match report language]
   - Tool results may contain content in various languages
   - If the mission requirement and source material are in Chinese, the report must be written entirely in Chinese
   - When citing English or mixed-language content from tools, translate it to match the report language
   - Preserve original meaning while ensuring natural and fluent expression
   - This rule applies to both body text and quoted blocks (> format)

4. [Faithfully present deliberation results]
   - Report content must reflect what the deliberation actually produced
   - Do not add information that does not exist in the deliberation data
   - If information is insufficient on a topic, state that explicitly
...
[Correct Example]
```
This section analyzes the wargaming results for COA 2. Based on in-depth analysis of the deliberation data, we found...

**Friendly Force Assessment**

The S3 Operations staff assessed that the main effort would require concentration of forces at the decisive point:

> "COA 2 provides the best opportunity for achieving tactical surprise at the objective..."

**Enemy Course of Action Analysis**

The S2 Intelligence section identified three most likely enemy COAs:

- ECOA 1: Defend in place with prepared positions
- ECOA 2: Counterattack from the north
- ECOA 3: Withdraw to secondary defensive belt
```

[Incorrect Example]
```
## Executive Summary          <- WRONG! Do not add any headings
### 1. Force Assessment       <- WRONG! Do not use ### subsections
...
[Tool Usage Recommendations - Use a mix of different tools, not just one]
- insight_forge: Deep tactical analysis, automatically decomposes questions and retrieves multi-dimensional deliberation facts and relationships
- panorama_search: Broad operational picture search, understand the full situation, timeline, and evolution of assessments
- quick_search: Quickly verify a specific intelligence point or finding
- interview_agents: Interview staff officer agents for first-person perspectives on the COA and their professional assessments
...
1. Content must be based on deliberation data retrieved via tools
2. Extensively cite original assessments and findings to substantiate the analysis
...
4. [Quote format - must be in its own paragraph]
   ...
   Correct format:
   ```
   The S2 Intelligence assessment highlighted significant risk in this area.

   > "Enemy reinforcement from the northern corridor could arrive within 4-6 hours of H-hour."

   This assessment was corroborated by the wargaming results.
   ```

   Incorrect format:
   ```
   The S2 assessment highlighted risk. > "Enemy reinforcement..." This was corroborated...
   ```
...
2. You must call tools to retrieve deliberation data before writing
...
2. Then call a tool (Action) to retrieve deliberation data
...
===============================================================
Tool calls used: {tool_calls_count}/{max_tool_calls} (tools used: {used_tools_str}){unused_hint}
- If information is sufficient: output section content starting with "Final Answer:" (must cite the above sources)
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    ...
    "Please call more tools to retrieve additional deliberation data before outputting Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only {tool_calls_count} tool calls made; at least {min_tool_calls} are required. "
    "Please call a tool to retrieve deliberation data. {unused_hint}"
)
...
REACT_UNUSED_TOOLS_HINT = "\nTip: You have not yet used: {unused_list}. Consider trying different tools for multi-angle intelligence."
...
You are a concise and efficient tactical analysis assistant.

[Background]
Mission/deliberation requirement: {simulation_requirement}
...
class ReportAgent:
    """
    Report Agent - Tactical Decision Report Generation Agent

    Uses a ReACT (Reasoning + Acting) pattern:
    1. Planning phase: Analyze the mission requirement, plan report structure
    ...
    def __init__(
        ...
        Args:
            ...
            simulation_id: Deliberation session ID
            simulation_requirement: Mission/deliberation requirement description
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    "query": "The tactical question or topic you want to analyze in depth",
            ...
            "panorama_search": {
                ...
                "parameters": {
                    ...
                    "include_expired": "Whether to include superseded/historical assessments (default True)"
                }
            },
            "quick_search": {
                "name": "quick_search",
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g., 'Get the S2 assessment of enemy most likely COA at OBJ ALPHA')",
                    "max_agents": "Maximum number of staff officers to interview (optional, default 5, max 10)"
                }
            }
        }
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Broad operational picture search
                query = parameters.get("query", "")
                ...
                # Staff officer interview - call the actual interview API to get staff officer agent responses
            ...
            # ========== Backward-compatible legacy tools (internally redirected) ==========
                ...
                # Redirect to insight_forge for more powerful retrieval
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Response may contain reasoning text + bare JSON; try to extract last JSON object
    ...
    def plan_outline(
        ...
        Uses LLM to analyze the mission requirement and plan report structure
        ...
        if progress_callback:
            progress_callback("planning", 0, "Analyzing mission requirement...")

        # First retrieve the deliberation context
        ...
        try:
            ...
            outline = ReportOutline(
                title=response.get("title", "Tactical Decision Support Report"),
            ...
            return ReportOutline(
                title="Tactical Decision Support Report",
                summary="MDMP deliberation results analysis with COA recommendation and risk assessment",
                sections=[
                    ReportSection(title="Mission Analysis and Intelligence Assessment"),
                    ReportSection(title="Course of Action Analysis and Wargaming Results"),
                    ReportSection(title="Risk Assessment and Recommendation")
                ]
            )
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section is included up to 4000 chars
        ...
        report_context = f"Section title: {section.title}\nMission requirement: {self.simulation_requirement}"
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    # Third time: degrade by truncating to first tool call, force execution
                    logger.warning(
            ...
            # Sufficient tool calls but LLM output content without "Final Answer:" prefix
            # Accept it directly as final answer to avoid spinning
        ...
        if response is None:
            ...
            final_answer = f"(Section generation failed: LLM returned empty response. Please retry.)"
    ...
    def generate_report(
        ...
        Each section is saved to file immediately upon completion, without waiting for the full report.
        File structure:
        reports/{report_id}/
        ...
        try:
            ...
            # Phase 2: Generate sections (save each section individually)
    ...
    def chat(
        ...
        # Get previously generated report content
        report_content = ""
        try:
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Start reading from this line (for incremental retrieval, 0 = from beginning)
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Keep original log line, strip trailing newlines
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if this duplicates the section title (within first 5 lines)
                if i < 5:
                ...
                # Section titles are added by the system; content should not have any headings
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep report title (#) and section headings (##), remove other heading levels (###, #### etc.)
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate heading (same title within last 5 lines)
                is_duplicate = False
    ...
    def get_report_by_simulation(cls, simulation_id: str) -> Optional[Report]:
        """Get report by simulation/deliberation ID"""
