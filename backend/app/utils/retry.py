            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"Function {func.__name__} still failing after {max_retries} retries: {str(e)}")
                        raise
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"Async function {func.__name__} still failing after {max_retries} retries: {str(e)}")
                        raise
